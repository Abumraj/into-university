...
Future showDownloadPopup(BuildContext context,
    {Video? video, String? videoUrl, String? description}) {
  assert(video != null || videoUrl != null);
  String? introVideoUrl;
  final yt = YoutubeExplode();
  Future<String?> getVideo() async {
    final stream = await yt.videos.streamsClient.getManifest(videoUrl!);
    yt.close();
    introVideoUrl = stream.muxed.bestQuality.url.toString();
...
class IntroVideoWidget extends StatefulWidget {
  ...
  const IntroVideoWidget(
      {Key? key, required this.url, required this.description})
      : super(key: key);
